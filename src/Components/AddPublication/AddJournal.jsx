...
const Add=()=>{
    ...
    const[visiISSN,setVisiISSN]=useState(false)
    const[visiTitle,setVisiTitle]=useState(false)
    const[visiVolume,setVisiVolume]=useState(false)
    const[visiCorrespondingAuthors,setVisiCorrespondingAuthors]=useState(false)
...
const formik1=useFormik({
    ...
    validate:values=>{
        ...
        }else if(values.ISSN.length<3){
            errors.ISSN='Atleast 3 characters required!'
        }
        else setData({...data,...values})
        // setData({values,...data})
      return errors;
    ...
    const formik=useFormik({
        ...
        validate:values=>{
           ...
           }else if(values.Volume.length<3){
            errors.Volume='Atleast 3 characters required!'
        }
           else setData({...data,...values})
...
useEffect(() => {
    ...
    if (formik1.errors.ISSN && formik1.touched.ISSN) {
        setVisiISSN(true);
    } else {
        setVisiISSN(false);
    }
...
useEffect(() => {
    ...
    if (formik.errors.Volume && formik.touched.Volume) {
        setVisiVolume(true);
    } else {
        setVisiVolume(false);
    }

    // if (formik.errors.FirstAuthor && formik.touched.FirstAuthor) {
    //     setVisiCorrespondingAuthors(true);
    // } else {
    //     setVisiCorrespondingAuthors(false);
    // }
    ...
    <div id='container'  className=" flex fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 z-40  bg-white rounded-lg shadow-2xl p-10 md:w-[50%] w-[92%] "> 
            ...
            <img src={remove} alt='remove' className='float-right  hover:opacity-10' onClick={navigateToHome}/>
                ...
                >
                 ...
                 visible={visiISSN}
                 placement='right'
                 content={formik1.errors.ISSN}
                >
                 <input type='text' className='border-2 border-[#bd8ce2] rounded-lg float-left mt-1 py-2 w-full' name='ISSN' onBlur={formik1.handleBlur} onChange={formik1.handleChange} value={formik1.values.ISSN} ></input>
                </Tippy>
                 <br/><br/>
                ...
                >
                    ...
                    <select className='bg-white border-2 border-[#bd8ce2] rounded-lg float-left mt-1 py-2 w-full' name='FirstAuthor' onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.FirstAuthor} >
                    ...
                    <select name='CorrespondingAuthor' className='bg-white border-2 border-[#bd8ce2] rounded-lg float-left mt-1 py-2 w-full'  onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.CorrespondingAuthor} >
                        <option selected  value="YES">Yes</option>
                       <option value="NO">No</option>
                    </select>
                 ...
                 visible={visiVolume}
                 placement='right'
                 content={formik.errors.Volume}
                >
                    <input type='text' className='border-2 border-[#bd8ce2] rounded-lg float-left mt-1 py-2 w-full' name='Volume' onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.Volume} ></input>
                 </Tippy>
                    <br/><br/>
