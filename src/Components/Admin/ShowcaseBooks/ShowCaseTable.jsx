...
const ShowCaseTable = (props) => {
  ...
  const[designation,setDesignation]=useState('');
    ...
    const url=`${process.env.REACT_APP_BASE_URL}home/faculty/book/admin/readBooks`;
   const fetchData=async ()=>{
    ...
    const showCaseData=table.filter((item,i)=>{
      ...
      if(search){
       ...
       if(( item.Name.toLowerCase().includes(search.toLowerCase())  || item.Phone.toString().includes(search.toLowerCase()) || item.BookTitle.toLowerCase().includes(search.toLowerCase()) || item.ISBN.toLowerCase().includes(search.toLowerCase())))
          // 
  ...
  return (
    <div className='min-h-screen h-max md:w-4/5 md:my-5 mx-auto'>
      {showCaseData?<Header publication='Book Records' data={table} />:null}
      <FilterBox 
      search={search}
      fromYear={fromYear}
      toYear={toYear}
      ...
      setDesignation={setDesignation} />
        ...
        <Link to='/admin/bookDetails/detailedBook'> 
