   ...
   const fetchData=async ()=>{
    ...
    const showCaseData=table.filter((item,i)=>{
      ...
      if(search){
       ...
       if(( item.Name.toLowerCase().includes(search.toLowerCase())  || item.Phone.toString().includes(search.toLowerCase()) || item.PaperTitle.toLowerCase().includes(search.toLowerCase()) || item.ISSN.toLowerCase().includes(search.toLowerCase())))
          // 
      ...
      // console.log("recieved confrernces ",item)
  ...
  return (
    <div className='min-h-screen h-max md:w-4/5 md:my-5 mx-auto'>
      ...
      setDesignation={setDesignation}/>
        ...
        <Link to='/admin/conferenceDetails/detailedConference'> 
